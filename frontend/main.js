      ...
      setTimeout(() => {
        ...
        if (proceedBtn) {
          proceedBtn.addEventListener('click', async function() {
            ...
            try {
              const res = await fetch('http://localhost:5000/api/demo_payment/doctor', {
                method: 'POST',
      ...
      setTimeout(() => {
        ...
        if (proceedBtn) {
          proceedBtn.addEventListener('click', async function() {
            ...
            try {
              const res = await fetch('http://localhost:5000/api/demo_payment/ai', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email })
              });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  daysOff = 3;
    ...
    if (downloadMedCert) {
      downloadMedCert.addEventListener('click', function() {
        ...
        fetch('http://localhost:5000/api/medical_certificate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function loadPatientDashboard() {
    const res = await fetch('/api/patient_dashboard', {
